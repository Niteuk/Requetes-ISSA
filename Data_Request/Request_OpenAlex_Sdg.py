...
MICROSERVICE_URL_SDG = config["MICROSERVICE"]["url_auth_sdg"]
...
# Call the microservice for SDG data
def fetch_sdg_data(document_uri, doi):
    """Call the microservice for SDG data"""
    try:
        ...
        url = f"{MICROSERVICE_URL_SDG}?documentUri={encoded_document_uri}&documentDoi={encoded_doi}&{encoded_sparql_query}"
        headers = {"Accept": "text/turtle"}

        logger.debug(f"Fetching SDG data for DOI {doi}")
        response = requests.get(url, headers=headers)
        response.raise_for_status()
        return response.text
    except requests.exceptions.HTTPError as e:
        logger.error(
            f"HTTPError: Could not add SDG data for DOI {doi}: {e.response.text}"
        )
        return None
    except requests.exceptions.RequestException as e:
        logger.error(f"RequestException: Could not add SDG data for DOI {doi}: {e}")
        return None
...
if USE_MAILTO:
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_doi = {
            executor.submit(fetch_sdg_data, doc_uri, doi): (doc_uri, doi)
        ...
        for future in tqdm(as_completed(future_to_doi), total=len(doi_list)):
            ...
            try:
                ...
                if rdf_data:
                    ...
                    logger.debug(f"SDG data for DOI {doi} recorded.")
                else:
                    error_count += 1
    ...
    for doi, doc_uri in tqdm(doi_list):
        try:
            rdf_data = fetch_sdg_data(doc_uri, doi)
            if rdf_data:
                rdf_results.append(rdf_data)
                logger.debug(f"SDG data for DOI {doi} recorded.")
            else:
...
output_path = config["output_paths"]["sdg_data"]
...
logger.info(f"SDG data saved in {output_path}")
