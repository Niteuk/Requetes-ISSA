...
def fetch_data(document_uri, doi):
    """
    Fetch SDGs for a given DOI and document URI
    ...
    try:
        ...
        url = f"{config['services']['sdg']}?documentUri={encoded_document_uri}&documentDoi={encoded_doi}&{encoded_sparql_query}"
        ...
        logger.error(f"Could not get SDGs for DOI {doi}: {e.response.text}")
        return None
    except requests.exceptions.RequestException as e:
        logger.error(f"Could not get SDGs for DOI {doi}: {e}")
        return None
